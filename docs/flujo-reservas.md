...
- **Special hours per date** (`schedule_overrides`, Configuración > Horarios > Horarios especiales): one row per barbershop+date, either closed all day or open with different hours (e.g. Navidad). It replaces the weekly `availability` for that date only. The admin can pick a single day or a date range (max 62 days); a range is stored as one row per day, and the list groups consecutive days with the same config back into a range (`groupOverrideRanges()`). The rule lives in `getDayAvailability()` (`src/lib/utils.ts`) and every slot/date picker uses it (booking flow, manual appointment modal, block-time modal, Instagram story). Saving one does **not** cancel existing appointments — the admin gets a warning listing the ones left outside the new hours.
- **Opening-hours validation**: `/api/appointments/create` rejects (HTTP 400) any block that falls outside the day's hours (weekly + special hours) via `isWithinOpeningHours()`. `/api/appointments/admin-create` does not validate this on purpose (walk-ins / admin authority).
