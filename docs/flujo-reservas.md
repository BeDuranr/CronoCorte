...
- **Special hours per date** (`schedule_overrides`, Configuración > Horarios > Horarios especiales): one row per barbershop+date, either closed all day or open with different hours (e.g. Navidad). It replaces the weekly `availability` for that date only. The rule lives in `getDayAvailability()` (`src/lib/utils.ts`) and every slot/date picker uses it (booking flow, manual appointment modal, block-time modal, Instagram story). Saving one does **not** cancel existing appointments — the admin gets a warning listing the ones left outside the new hours.
